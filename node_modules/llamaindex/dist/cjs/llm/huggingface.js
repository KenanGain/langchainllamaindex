...
class HuggingFaceLLM extends _llms.BaseLLM {
    ...
    async getTokenizer() {
        const { AutoTokenizer } = await (0, _env.loadTransformers)((transformer)=>{
            _Settings.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });
        if (!this.tokenizer) {
    ...
    async getModel() {
        const { AutoModelForCausalLM } = await (0, _env.loadTransformers)((transformer)=>{
            _Settings.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });

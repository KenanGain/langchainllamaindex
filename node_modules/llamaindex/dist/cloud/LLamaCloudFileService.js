import { FilesService, PipelinesService, ProjectsService } from "@llamaindex/cloud/api";
...
export class LLamaCloudFileService {
   ...
   */ static async getAllProjectsWithPipelines() {
        ...
        try {
            ...
            const { data: pipelines } = await PipelinesService.searchPipelinesApiV1PipelinesGet({
                throwOnError: true
            });
   ...
   */ static async addFileToPipeline(projectId, pipelineId, uploadFile, customMetadata = {}) {
        ...
        while(attempt < maxAttempts){
            const { data: result } = await PipelinesService.getPipelineFileStatusApiV1PipelinesPipelineIdFilesFileIdStatusGet({
                path: {
                    pipeline_id: pipelineId,
                    file_id: file.id
                },
   ...
   */ static async getFileUrl(pipelineId, filename) {
        initService();
        const { data: allPipelineFiles } = await PipelinesService.listPipelineFilesApiV1PipelinesPipelineIdFilesGet({
            path: {
                pipeline_id: pipelineId
            },
            throwOnError: true
        ...
        const { data: fileContent } = await FilesService.readFileContentApiV1FilesIdContentGet({
            path: {
                id: file.file_id
            },
            query: {

...
export class LlamaCloudIndex {
    ...
    async waitForPipelineIngestion(verbose = Settings.debug, raiseOnError = false) {
        ...
        while(true){
            const { data: pipelineStatus } = await PipelinesService.getPipelineStatusApiV1PipelinesPipelineIdStatusGet({
                path: {
    ...
    async waitForDocumentIngestion(docIds, verbose = Settings.debug, raiseOnError = false) {
        ...
        while(pendingDocs.size){
            ...
            for (const doc of pendingDocs){
                const { data: { status } } = await PipelinesService.getPipelineDocumentStatusApiV1PipelinesPipelineIdDocumentsDocumentIdStatusGet({
                    path: {
                        pipeline_id: pipelineId,
                        document_id: doc
                    },
   ...
   */ static async fromDocuments(params, config) {
    ...
    async ensureIndex(config) {
        ...
        if (pipelines.length === 0) {
            ...
            const { data: pipeline } = await PipelinesService.upsertPipelineApiV1PipelinesPut({
                path: {
                    project_id: projectId
                },
                body: {

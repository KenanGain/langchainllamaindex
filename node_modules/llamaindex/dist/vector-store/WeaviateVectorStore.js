...
import { metadataDictToNode, nodeToMetadata, parseArrayValue, parseNumberValue } from "./utils.js";
const NODE_SCHEMA = [
    {
        dataType: [
            "text"
...
const buildFilterItem = (collection, filter)=>{
    ...
    switch(operator){
        ...
        case ">":
            {
                return collection.filter.byProperty(key).greaterThan(parseNumberValue(value));
            }
        case "<":
            {
                return collection.filter.byProperty(key).lessThan(parseNumberValue(value));
            }
        case ">=":
            {
                return collection.filter.byProperty(key).greaterOrEqual(parseNumberValue(value));
            }
        case "<=":
            {
                return collection.filter.byProperty(key).lessOrEqual(parseNumberValue(value));
            }
        case "any":
            {
                return collection.filter.byProperty(key).containsAny(parseArrayValue(value).map(String));
            }
        case "all":
            {
                return collection.filter.byProperty(key).containsAll(parseArrayValue(value).map(String));
            }
        default:
            {
...
export class WeaviateVectorStore extends VectorStoreBase {
    storesText = true;
    flatMetadata = true;
    weaviateClient;
    clusterURL;
    ...
    constructor(init){
        ...
        if (init?.weaviateClient) {
            ...
            const clusterURL = init?.cloudOptions?.clusterURL ?? getEnv("WEAVIATE_CLUSTER_URL");

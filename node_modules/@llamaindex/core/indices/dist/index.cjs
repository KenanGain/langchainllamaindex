...
var index_cjs$1 = require('../../node-parser/dist/index.cjs');
 ...
 */ class PromptHelper {
    constructor(options = {}){
        const { contextWindow = index_cjs$2.DEFAULT_CONTEXT_WINDOW, numOutput = index_cjs$2.DEFAULT_NUM_OUTPUTS, chunkOverlapRatio = index_cjs$2.DEFAULT_CHUNK_OVERLAP_RATIO, chunkSizeLimit, tokenizer, separator = " " } = options;
        this.contextWindow = contextWindow;
        this.numOutput = numOutput;
        this.chunkOverlapRatio = chunkOverlapRatio;
        this.chunkSizeLimit = chunkSizeLimit;
        this.tokenizer = tokenizer ?? env.tokenizers.tokenizer();
   ...
   */ #getAvailableChunkSize(prompt, numChunks = 1, padding = 5) {
        ...
        if (prompt instanceof index_cjs.PromptTemplate) {
   ...
   */ getTextSplitterGivenPrompt(prompt, numChunks = 1, padding = index_cjs$2.DEFAULT_PADDING) {
        ...
        return new index_cjs$1.TokenTextSplitter({
   ...
   */ truncate(prompt, textChunks, padding = index_cjs$2.DEFAULT_PADDING) {
        const textSplitter = this.getTextSplitterGivenPrompt(prompt, textChunks.length, padding);
        return textChunks.map((chunk)=>index_cjs$1.truncateText(chunk, textSplitter));
    }
    /**
   * Repack text chunks to better utilize the available context window.
   */ repack(prompt, textChunks, padding = index_cjs$2.DEFAULT_PADDING) {
    ...
    static fromLLMMetadata(metadata, options) {
        const { chunkOverlapRatio = index_cjs$2.DEFAULT_CHUNK_OVERLAP_RATIO, chunkSizeLimit = index_cjs$2.DEFAULT_CHUNK_SIZE, tokenizer = index_cjs$2.Settings.tokenizer, separator = " " } = options ?? {};
        return new PromptHelper({
            contextWindow: metadata.contextWindow,
            // fixme: numOutput is not in LLMMetadata
            numOutput: index_cjs$2.DEFAULT_NUM_OUTPUTS,
            chunkOverlapRatio,
            chunkSizeLimit,
            tokenizer,
            separator
